...
from util.thuman import ThumanDataset
...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/s3dis/thuman_pointtransformer_repro.yaml', help='config file')
    parser.add_argument('opts', help='see config/s3dis/thuman_pointtransformer_repro.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def main():
    ...
    if args.data_name == 'thuman':
        ThumanDataset(split='train', data_root=args.data_root, test_area=args.test_area)
        ThumanDataset(split='val', data_root=args.data_root, test_area=args.test_area)
    else:
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    train_data = ThumanDataset(split='train', data_root=args.data_root, test_area=args.test_area, voxel_size=args.voxel_size, voxel_max=args.voxel_max, transform=train_transform, shuffle_index=True, loop=args.loop)
    if main_process():
    ...
    if args.evaluate:
        val_transform = None
        val_data = ThumanDataset(split='val', data_root=args.data_root, test_area=args.test_area, voxel_size=args.voxel_size, voxel_max=800000, transform=val_transform)
        if args.distributed:
